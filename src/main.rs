...
use utils::{
	get_auth_ix, get_clock, get_config_and_proof, get_cutoff, get_cutoff_with_risk, get_mine_ix,
	get_proof, get_register_ix, get_reset_ix, proof_pubkey, ORE_TOKEN_DECIMALS,
};
					...
					if solution.is_some() {
						...
						for i in 0..SUBMIT_LIMIT {
							if let Some(best_solution) = best_solution {
								...
								info!("Getting current config and proof.");
								...
								let fee: u64 = if *app_dynamic_fee {
									...
									.await
									{
										Ok(fee) => {
											...
											// MI: calc uplimit of priority fee for precious fee difficulty, e.g. diff > 27
											{
