...
enum FeeStrategy {
    ...
    pub async fn dynamic_fee(rpc_client: RpcClient, dynamic_fee_url: Option<String>, priority_fee_cap: Option<u64>) -> Result<u64, String> {
        // Get rpc url for fee estimate
        let rpc_url = if let(dynamic_fee_url) = dynamic_fee_url {
            dynamic_fee_url
        } else {
            rpc_client.url()
        };
        ...
        let calculated_fee = match strategy {
            ...
            FeeStrategy::LOCAL => {
                local_dynamic_fee().await.or_else(|err| {
